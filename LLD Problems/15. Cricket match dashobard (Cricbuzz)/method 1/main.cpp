// ChainStrikingPlayerStrategy is having vector<IStrikingPlayerStrategy> 
#include <iostream>
#include <vector>
#include <string>
#include <memory>
...
public:
    ...
    int getTotalWickets() const {
        return 0; // Logic can be expanded if Wickets are tracked
    }
...
protected:
    ...
    bool isApplicable(const MatchState& state, const Ball& ball) const override {
...
private:
    vector<unique_ptr<IStrikingPlayerStrategy>> chain;
...
protected:
    void _updateMatchState(MatchState& state, const Ball& ball) override {
        for (auto& strategy : chain) {
            strategy->updateMatchState(state, ball);
        }
    }
...
public:
    ...
    void setupStrategies() {
        ...
        chain.addStrategy(make_unique<WicketBasedStrikingPlayerStrategy>());
        chain.addStrategy(make_unique<OverChangeStrikingPlayerStrategy>());
    }

    void handleThrowBall(const Ball& ball) {
        scoreService.handleThrowBall(ball.bowler, state.strikingPlayer, ball);
        chain.updateMatchState(state, ball);
    }
...
int main() {
    ...
    // Simulate Balls
    matchService.handleThrowBall(Ball(1, BallType::NORMAL, Run(1, RunType::PLAYER_RUN), &bowler));
    matchService.handleThrowBall(Ball(2, BallType::NORMAL, Run(4, RunType::FOUR), &bowler));
    matchService.handleThrowBall(Ball(3, BallType::WICKET, Run(), &bowler));
    matchService.handleThrowBall(Ball(6, BallType::NORMAL, Run(2, RunType::PLAYER_RUN), &bowler));
